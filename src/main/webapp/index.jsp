...
<%--    <a href="http://localhost:8080/detail?mgrNo=<%= createTable(data) %>">
        ...
        if (data.length === 0) { // 데이터가 없으면
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < data[i].length; j++) {
                    var td = document.createElement('td');
                    if(j == 3){
                        var nm = data[i][j]; // 와아파이명
                        var no = data[i][0]; // 와이파이 고유번호

                        // 동적으로 <a> 태그를 생성하여 링크 삽입
                        var link = document.createElement('a');
                        link.href = 'http://localhost:8080/detail';//'?mgrNo=' + no;
                        link.innerText = nm;
                        td.appendChild(link);
                    } else{
